...
rewards['source-source reward'] = []
rewards['source-target reward'] = []
...
for meta_dir, model_dir in zip(metas, models):
    ...
    print(f'loading model {model_dir.split("/")[4]} with meta {meta_dir.split("/")[5]}')
    envs = {
        'source': env_source,
        'target': env_target
    }
    ...
    for env in envs.items():
        ...
        print(f'mean reward in source-{env[0]}: {mean_reward}')
        env[1].close()
        
        if env[0]=='target':
            model_dir = model_dir.replace('source', 'target')
            model = PPO.load(model_dir, env=env[1])
            mean_reward, std_reward = evaluate_policy(model,
                                                      model.get_env(),
                                                      n_eval_episodes=eval_ep)
    ...
    rewards['source-source reward'].append(source_target_reward['source'])
    rewards['source-target reward'].append(source_target_reward['target'])
